...
public class PathFindBuffers
{
    public static class BufferInfo
    {
        ...
        final int index;
        private int overBuffers;
        private int totalUses;
        private double useTimeMillis;
        private FastTable<PathFindBuffer> buffers;
	...
	public BufferInfo(int mapSize, int idx, int buffersCount)
        {
            ...
            totalUses = 0;
            useTimeMillis = 0.0D;
            buffers = new FastTable<PathFindBuffer>(buffersCount);
            MapSize = mapSize;
    ...
    public static class PathFindBuffer
    {
        ...
        public synchronized void free()
        {
            ...
            currentNode = null;
            info.totalUses++;
            info.useTimeMillis += (double)(System.nanoTime() - useStartedNanos) / 1000000D;
            inUse = false;
        }
    ...
    {
        FastMap<Integer, Integer> conf_data = new FastMap<Integer, Integer>();
        String arr$[] = s.split(";");
        int len$ = arr$.length;
        for(int i$ = 0; i$ < len$; i$++)
        {
        ...
        for(int i = 0; i < all_buffers.length; i++)
            if(MapSize == all_buffers[i].MapSize)
            {
                if(newCapacity == all_buffers[i].buffers.size())
                    return true;
                FastTable<PathFindBuffer> new_buffers = new FastTable<PathFindBuffer>(newCapacity);

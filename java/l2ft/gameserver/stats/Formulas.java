...
public class Formulas
{
	...
	public static boolean calcBlow(Creature activeChar, Creature target, Skill skill)
	{
		...
		{
			activeChar.sendMessage(new CustomMessage("l2ft.gameserver.skills.Formulas.Chance", (Player)activeChar, new Object[0]).addString(skill.getName()).addNumber((long) chance));
		}
	...
	public static boolean calcSkillSuccess(Env env, EffectTemplate et, int spiritshot)
	{
		...
		int mLevel = skill.getMagicLevel() == 0 ? caster.getLevel() : skill.getMagicLevel(); // Разница в уровнях
		if(mLevel > 0)
		{
			...
			env.value += deltamod;  // -50 от общего шанса.
		}
		...
		if(caster.isPlayer() && (Config.SKILLS_CHANCE_SHOW || caster.getPlayer().isGM()))
		{
			...
			{
				player.sendMessage(new CustomMessage("l2ft.gameserver.skills.Formulas.Chance", player, new Object[0]).addString(skill.getName()).addNumber(Math.round(env.value)));
			}
		}
		if(caster.isMonster())
			env.value *= Config.SKILLS_MOB_CHANCE;

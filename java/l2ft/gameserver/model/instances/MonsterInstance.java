...
public class MonsterInstance extends NpcInstance
{
	...
	{
		if(!isBoss() && !isRaid() && !(this instanceof ReflectionBossInstance) && isLeader() && getMinionList().hasAliveMinions())
		{
			if(_leaderTask == null)
			{
				if(isMonster() && (this.isSeeded() || this.isSpoiled()))
					startDecayLeader(20000L);
				else if(isBoss())
					startDecayLeader(20000L);
				else if(isFlying())
					startDecayLeader(4500L);
				else
					startDecayLeader(8500L);
			}
			_leaderTask = ThreadPoolManager.getInstance().schedule(new leaderTask(), 5000);
			return;
		}
		super.onDecay();
	}

	private ScheduledFuture<?> _decayLeaderTask = null;
	public class DecayLeaderTask extends RunnableImpl
	{
		@Override
		public void runImpl() throws Exception
		{
			if(_decayLeaderTask != null)
			{
				_decayLeaderTask.cancel(false);
				_decayLeaderTask = null;
			}
			removeVisible();
		}
	}
	 ...
	 */
	protected void startDecayLeader(long delay)
	{
		_decayLeaderTask = ThreadPoolManager.getInstance().schedule(new DecayLeaderTask(), delay);
	}

	private ScheduledFuture<?> _leaderTask = null;
	public class leaderTask extends RunnableImpl
	{
		@Override
		public void runImpl() throws Exception
		{
			if(isLeader() && !getMinionList().hasAliveMinions())
				_leaderTask = null;
			onDecay();
		}
	}

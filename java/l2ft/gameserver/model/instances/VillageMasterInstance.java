...
import l2ft.gameserver.model.SubClass;
import l2ft.gameserver.model.base.ClassId;
import l2ft.gameserver.model.base.ClassType;
import l2ft.gameserver.model.base.PlayerClass;
...
public final class VillageMasterInstance extends NpcInstance
{
		...
		else if(command.startsWith("Subclass"))
		{
			...
			switch(intVal)
			{
				case 1: // Возвращает список сабов, которые можно взять (см case 4)
					...
					{
						player.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.NoSubAtThisTime", player));
						return;
					}
					break;
				...
				case 4: // Добавление сабкласса - обработка выбора из case 1
					...
					{
						player.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.NoSubBeforeLevel", player).addNumber(Config.ALT_GAME_LEVEL_TO_GET_SUBCLASS));
						allowAddition = false;
					}
					...
					if(!Config.ALT_GAME_SUBCLASS_WITHOUT_QUESTS && !playerClassList.isEmpty() && playerClassList.size() < 2 + Config.ALT_GAME_SUB_ADD)
						if(player.isQuestCompleted("_234_FatesWhisper"))
						{
							if(player.getRace() == Race.kamael)
							{
								...
								if(!allowAddition)
									player.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.QuestSeedsOfChaos", player));
							}
							else
							{
								allowAddition = player.isQuestCompleted("_235_MimirsElixir");
								if(!allowAddition)
									player.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.QuestMimirsElixir", player));
							}
						}
						else
						{
							player.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.QuestFatesWhisper", player));
							allowAddition = false;
						}
					...
					if(allowAddition)
					{
						...
						{
							player.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.SubclassCouldNotBeAdded", player));
							return;
						}
	...
	public void setLeader(Player leader, String newLeader)
	{
		...
		if(member == null)
		{
			...
			//leader.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.S1IsNotMemberOfTheClan", leader).addString(newLeader));
		...
		{
			leader.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.CannotAssignUnitLeader", leader));
			return;
		}
	...
	{
		player.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.ClanLeaderWillBeChangedFromS1ToS2", player).addString(clan.getLeaderName()).addString(newLeader.getName()));
	...
	public void createSubPledge(Player player, String clanName, int pledgeType, int minClanLvl, String leaderName)
	{
		...
		if(pledgeType != Clan.SUBUNIT_ACADEMY)
		{
			...
			if(subLeader == null)
			{
				player.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.PlayerCantBeAssignedAsSubUnitLeader", player));
				return;
			}
			else if(subLeader.getLeaderOf() != Clan.SUBUNIT_NONE)
			{
				player.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.ItCantBeSubUnitLeader", player));
				return;
			}
		}
	...
	public void assignSubPledgeLeader(Player player, String clanName, String leaderName)
	{
		...
		if(clan == null)
		{
			player.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.ClanDoesntExist", player));
			return;
		}
		...
		if(targetUnit == null)
		{
			player.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.SubUnitNotFound", player));
			return;
		}
		SubUnit mainUnit = clan.getSubUnit(Clan.SUBUNIT_MAIN_CLAN);
		...
		player.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.NewSubUnitLeaderHasBeenAssigned", player));
	}

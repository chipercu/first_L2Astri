...
import l2ft.gameserver.skills.skillclasses.CurseDivinity;
...
import l2ft.gameserver.skills.skillclasses.DisablersRoot;
...
import l2ft.gameserver.skills.skillclasses.SummonHealPercent;
import l2ft.gameserver.skills.skillclasses.InstantJump;
import l2ft.gameserver.skills.skillclasses.KamaelWeaponExchange;
import l2ft.gameserver.skills.skillclasses.LearnSkill;
...
import l2ft.gameserver.skills.skillclasses.SummonManaHealPercent;
...
import l2ft.gameserver.skills.skillclasses.PcBangPointsAddrnd;
...
public abstract class Skill extends StatTemplate implements Cloneable
{
	...
	public SystemMsg checkTarget(Creature activeChar, Creature target, Creature aimingTarget, boolean forceUse, boolean first)
	{
		...
		{
			if(!PositionUtils.isFacing(activeChar,  target,  getCastRange())) //FIX SkillID:3318 - Range (Thx! KiBerGen || Fix:Payne)
				return SystemMsg.YOUR_TARGET_IS_OUT_OF_RANGE;
			else
				return null;
		}
				...
				if((target.isPlayer() || target.isPet() || target.isSummon()) && (activeChar.isPlayer() || activeChar.isPet() || activeChar.isSummon()))
				{
					...
					for(GlobalEvent e : activeChar.getEvents())
						if(e != null)
							if(!e.canAttack(target, activeChar, this, forceUse) && activeChar.isInZone(ZoneType.SIEGE))
								return false;
					// DS: массовые атакующие скиллы не игнорируют группу
					if(target.getPlayer().isInParty() && activeChar.getPlayer().isInParty() && activeChar.getPlayer().getParty() == target.getPlayer().getParty())
						return false;
					// DS: массовые атакующие скиллы не игнорируют клан
					if(activeChar.getPlayer().getClan() != null && target.getPlayer().getClan() != null && activeChar.getPlayer().getClanId() == target.getPlayer().getClanId())
						return false;
					// DS: массовые атакующие скиллы не игнорируют командный канал и альянс
					if(activeChar.getPlayer().isInParty() && activeChar.getPlayer().getParty().getCommandChannel() != null && target.getPlayer().isInParty() && target.getPlayer().getParty().getCommandChannel() != null && activeChar.getPlayer().getParty().getCommandChannel() == target.getPlayer().getParty().getCommandChannel())
						return false;
	...
	public final void getEffects(Creature effector, Creature effected, boolean calcChance, boolean applyOnCaster, boolean skillReflected)
	{
...

        {
			...
			getEffects(effector, effected, calcChance, applyOnCaster, timeConst, timeMult, skillReflected);
        }

...
import l2ft.gameserver.network.authcomm.gspackets.ChangeAllowedHwid;
import l2ft.gameserver.network.authcomm.gspackets.ChangeAllowedIp;
...
public class Security implements IVoicedCommandHandler
{
	...
	public boolean useVoicedCommand(String command, Player activeChar, String target)
	{
		...
		else if(command.equalsIgnoreCase("lockHwid"))
		{
			...
			AuthServerCommunication.getInstance().sendPacket(new ChangeAllowedHwid(activeChar.getAccountName(), activeChar.getHWID().Full));
			NpcHtmlMessage html = new NpcHtmlMessage(activeChar.getObjectId());

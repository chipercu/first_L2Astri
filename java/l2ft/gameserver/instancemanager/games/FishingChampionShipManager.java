			...
			for(Fisher fisher : _tmpPlayers)
				if(fisher.getName().equalsIgnoreCase(pl.getName()))
				{
					if(fisher.getLength() < len)
					{
						...
						pl.sendMessage(new CustomMessage("l2ft.gameserver.instancemanager.games.FishingChampionShipManager.ResultImproveOn", pl));
						recalculateMinLength();
					}
					return;
				}
			_tmpPlayers.add(new Fisher(pl.getName(), len, 0));
			pl.sendMessage(new CustomMessage("l2ft.gameserver.instancemanager.games.FishingChampionShipManager.YouInAPrizeList", pl));
			recalculateMinLength();
		}

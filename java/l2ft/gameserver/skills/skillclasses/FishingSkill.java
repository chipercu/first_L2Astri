...
public class FishingSkill extends Skill
{
	...
	public boolean checkCondition(Creature activeChar, Creature target, boolean forceUse, boolean dontMove, boolean first)
	{
		...
		if(!isInWater && !player.isInZone(ZoneType.FISHING))
		{
			player.sendPacket(SystemMsg.YOU_CANT_FISH_HERE);
			return false;
		}
	...
	public void useSkill(Creature caster, List<Creature> targets)
	{
		...
		int group = l2ft.gameserver.model.Fishing.getFishGroup(lure.getItemId());
		int type = l2ft.gameserver.model.Fishing.getRandomFishType(lureId);
		int lvl = l2ft.gameserver.model.Fishing.getRandomFishLvl(player);

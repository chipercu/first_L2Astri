...
public class ManageProf implements ScriptFile, ICommunityBoardHandler
{
		...
		if(command.startsWith("_bbscareer;sub;"))
		{
			...
			switch(intVal)
			{
				...
				case 4: // Добавление сабкласса - обработка выбора из case 1
					...
					{
						activeChar.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.NoSubBeforeLevel", activeChar).addNumber(Config.ALT_GAME_LEVEL_TO_GET_SUBCLASS));
						allowAddition = false;
					}
				...
				case 6: // Отмена сабкласса - обработка выбора из case 3
					...
					{
						activeChar.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.NoSubAtThisTime", activeChar));
						return;
					}
					break;
				case 7: // Отмена сабкласса - обработка выбора из case 6
					...
					{
						activeChar.sendMessage(new CustomMessage("l2ft.gameserver.model.instances.L2VillageMasterInstance.SubclassCouldNotBeAdded", activeChar));
						return;
					}
					break;
			}
	...
	private StringBuilder getSubClassesHtml(Player activeChar, boolean condition)
	{
		...
		if(subsAvailable != null && !subsAvailable.isEmpty() && condition/* && Config.BBS_PVP_SUB_MANAGER_ALLOW*/) //TODO
		{
